/* eslint-disable function-paren-newline */ /* eslint-disable no-shadow */ "use strict";
// #region constants
...
// #endregion
// #region validators
const CHECK_EMPLOYEE_AGE = (age)=>age >= EMPLOYEE_MIN_AGE && age <= EMPLOYEE_MAX_AGE;
...
function handleHeadingClick(index) {
    ...
    rowsCopy.sort((a, b)=>{
        ...
        if (!isNaN(+aTextValue + +bTextValue)) return +aTextValue - +bTextValue;
        return aTextValue.localeCompare(bTextValue);
    });
    if (reverseOrder) rowsCopy.reverse();
    for (const row of rowsCopy)tableBody.appendChild(row);
}
function handleSelectTableRow(ev) {
    const row = ev.target.closest("tr");
    if (selectedRow) selectedRow.classList.remove(ROW_ACTIVE_CLASS);
    selectedRow = row;
...
function handleFormSubmit(ev) {
    ...
    if (!CHECK_EMPLOYEE_NAME(employee.name)) {
        pushNotification("Error!", "Employee name length should be 4 symbols or longer!");
        return;
    }
    ...
    const cells = employeeToAppend.map((value)=>createHtmlElement("td", {
            textContent: value
        }));
...
function createHtmlElement(tag, props) {
    ...
    if (props && Object.keys(props).length > 0) {
        ...
        if ((tag === "input" || tag === "select") && props.name) element.setAttribute("data-qa", props.name);
    }
    return element;
}
...
function createSelectInput(labelProps, selectProps) {
    ...
    appendChildren(selectInput, OFFICE_OPTIONS.map((option)=>createHtmlElement("option", {
            value: option,
            textContent: option
        })));
